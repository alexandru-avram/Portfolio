...
def get_random_cafe():
    ...
    # To return the whole data: return jsonify(cafe=random_cafe.to_dict())
...
def search_location():
    ...
    # GET restults where location matches query location
    result = db.session.execute(db.select(Cafe).order_by(Cafe.name).where(Cafe.location == query_location))

    all_cafes = result.scalars().all()

    # If we find cafes, return them, else return an error
    if all_cafes:
        return jsonify(cafes=[cafe.to_dict() for cafe in all_cafes])
    else:
